...
if __name__ == '__main__':
    logfilename = "Presidio_Moniotring%s.log"%(strftime("%Y%m%d%H%M", gmtime()))
    logger = set_log(logfilename,"Presidio_Moniotring")
    ip ="10.2.53.161"
    port = 22
    mode ="ssh"
    username = "admin"
    password ="admin"
    device =Device_Tool(ip,port,mode,username,password,"Presidio_Moniotring")
    test_cycle = 350
    if device:
